...
const createUser = async function (abcd, xyz) {
  ...
  try {
    ...
    if (Object.keys(data).length !== 0) {
      ...
      return xyz.status(201).send({ status: true, msg: savedData })
    }
    let savedData = await userModel.create(data);
...
const loginUser = async function (req, res) {
  ...
  try {
    let data = req.body
    if (Object.keys(data).length === 0) {
      return res.status(400).send({ msg: "username & password is required" })
    }
...
const getUserData = async function (req, res) {
  try {
    ...
    xyz.send({ msg: "Error", error: err.message })
  }
}
...
const postMessage = async function (req, res) {
  try {
    ...
    let updatedUser = await userModel.findOnendUpdate({ _id: user._id }, { posts: updatedPosts }, { new: true })

    //return the updated user document
    return res.send({ status: true, data: updatedUser })
  }
  catch (err) {
    res.status(500).send({ msg: "Error", error: "post is not possible" })
  }
}
