...
const createBooks = async (req, res) => {
    try {
        ...
        if (subcategory.length == 0)
            return res.status(400).send({ status: false, message: "Please provide valid type or it cannot remains empty" });
...
const getBooksById = async function (req, res) {
    try {
        ...
        if (!bookIdCheck) return res.status(404).send({ status: false, message: "No book present from this BOOKID" });
