...
const shortUrl=async function(req,res){
    ...
    if(!regex.test(longUrl)) return res.status(400).send({status:false,message:"LongUrl is invalid"})
    const foundUrl=await urlModel.findOne({longUrl})
    if(foundUrl) return res.status(400).send({status:false,message:"This url is already exist"})
...
const urlCode=async function(req,res){
    ...
    res.status(308).redirect(foundCode.longUrl)
}
