...
const authorisation = async function (req, res, next) {
    try {
        ...
        if (bookId) {
            ...
            if (!objectValue(usersId))
                return res.status(400).send({ status: false, message: "Please enter userId" });

            if (!isValidObjectId(usersId))
                return res.status(400).send({ status: false, message: "UserId is not in correct formate" });

            //  if (!usersId)
            //      return res.status(400).send({ status: false, message: "body should not remain empty" });

            if (userLoggedIn != usersId) return res.send({ status: false, message: 'User logged is not allowed to modify the requested users data' })
        }
        next();
    } catch (err) {
        res.status(500).send({ status: false, message: err.message })
